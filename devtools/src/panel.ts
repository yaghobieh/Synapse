...
(function() {
  ...
  function setupTabs(): void {
    tabButtons.forEach(button => {
      button.addEventListener('click', () => {
        ...
        const panelElement = document.getElementById(`${tabName}-panel`);
        if (panelElement) {
  ...
  function setConnected(connected: boolean): void {
    ...
    const statusDot = connectionStatus.querySelector('.status-dot') as HTMLSpanElement;
  ...
  function renderActions() {
    ...
    actionsList.innerHTML = actions.map((action, index) => `
      <div class="action-item ${index === selectedActionIndex ? 'selected' : ''}" 
        ...
        <span class="action-type">${action.type}</span>
          <span class="action-time">${formatTime(action.timestamp || Date.now())}</span>
      </div>
    `).join('');
    ...
    actionsList.querySelectorAll('.action-item').forEach(item => {
      item.addEventListener('click', () => {
        ...
        selectedActionIndex = index ? parseInt(index) : -1;
        renderActions();
  ...
  function renderActionDetail(action: Action | null): void {
    if (!action) {
  ...
  // Render diff
  function renderDiff(actionIndex: number): void {
  ...
  // Execute API action
  function executeApiAction(): void {
    const methodElement = document.getElementById('api-method') as HTMLSelectElement;
    const urlElement = document.getElementById('api-url') as HTMLInputElement;
    const actionTypeElement = document.getElementById('api-action-type') as HTMLInputElement;
    const bodyElement = document.getElementById('api-body') as HTMLTextAreaElement;
